...
- [x] **Task 1** — `backend/app/worker/settings.py`: `PipelineSettings` (queue `arq:pipeline`, `max_jobs=20`, 120s timeout) and `InteractiveSettings` (queue `arq:interactive`, `max_jobs=5`, 60s timeout) per §8. Only `enrich_company` registered on `InteractiveSettings` for now.
- [x] **Task 2** — `backend/app/services/enrichment.py`: builds the `gemini-2.5-flash` call (structured output, `temperature=0.2`) from name/domain/cached homepage text; handles §7.2's edge-case table (`is_known`/`confidence` → `warnings=['low_confidence']`, no competitors → `warnings=['no_competitors']`, **never blocks**); reads/writes `cache:enrich:{domain}` (7d TTL, gated on `confidence ≥ 0.7`).
- [x] **Task 3** — `backend/app/llm/prompts/enrichment.jinja`: the enrichment prompt, delimits scraped homepage text as untrusted per §14's prompt-injection mitigation.
- [x] **Task 4** — `backend/app/llm/schemas.py`: add the enrichment structured-output schema (industry, products, competitors, aliases, description, keywords, `is_known`, `confidence`).
- [x] **Task 5** — `backend/app/worker/jobs.py`: `enrich_company(ctx, scan_id)` — loads scan/company, calls `enrichment.enrich(...)`, writes `company_profiles` v1, uses Phase 1's `lifecycle.transition()` to move `scans.status` `enriching → awaiting_verification` (or `failed`).
- [x] **Task 6** — Wire the deferred `enqueue_job("enrich_company", scan_id, ...)` into `POST /scans` from Phase 3.
- [x] **Task 7** — `backend/app/core/progress.py`: `publish(scan_id)` helper writing the `scan:{id}:progress` Redis hash (§9); `GET /scans/{id}` in `scans.py` reads it per §6's "Progress delivery". Every later job calls `progress.publish()`.
...
- [x] **Task 1** — `backend/app/api/v1/profiles.py`: `GET /scans/{id}/profile`, `PATCH /scans/{id}/profile` (writes `company_profiles` v2, `source='user_edited'` — industry/products/competitors/aliases/description only; **company and website are not accepted fields here**, per the resolved gap), `POST /scans/{id}/profile/confirm` (enqueues `verify_profile`).
- [x] **Task 2** — `backend/app/services/verification.py`: `verify_profile` logic — sends the user's profile to `gemini-2.5-flash` as critic, parses `{verdict, issues}`; `issues_found` → back to `awaiting_verification` with `company_profiles.issues` populated; second confirm → writes v3 (`source='ai_verified'`).
- [x] **Task 3** — `backend/app/llm/prompts/verification.jinja`: the critic prompt per §7.3.
- [x] **Task 4** — `backend/app/services/entity_resolution.py`: normalization only for now (lowercase/NFKD-fold/strip-suffix/strip-punctuation per §11) plus the freeze step — on acceptance, flatten target + aliases + product names + competitors into `scan_entities` rows (`is_target` set correctly). Full match-order logic (fuzzy matching, word-boundary guard, discovered-name dedupe) is built in Phase 9, where it's first exercised against real response text.
- [x] **Task 5** — `backend/app/worker/jobs.py`: add `verify_profile(ctx, scan_id)` to `InteractiveSettings`.
...
- [x] **Task 1** — `backend/app/api/v1/scans.py`: `PUT /scans/{id}/scope` — validates against the fixed 9-category enum from PRD Phase 4, defaults to all 9, writes `scans.monitoring_categories`, `409 INVALID_STATE_TRANSITION` (via `lifecycle.transition()`) unless status is `scope_pending`.
...
- [x] **Task 1** — `backend/app/services/execution.py`: per §8's pseudocode — cancellation check (`scan:{id}:cancelled`), `pool.acquire()` from `google_exec`/`groq_exec`, provider adapter call (60s timeout), **upsert** `ai_responses` on `(prompt_id, provider)`, on success enqueue `evaluate_response` with `_job_id=f"eval:{resp.id}"` (stub call — body lands in Phase 9), `DECR pending_exec`/`pending_eval` via a shared `_decr` helper in `app/core/progress.py` (reused by Phase 9).
- [x] **Task 2** — `backend/app/worker/jobs.py`: `execute_prompt(ctx, scan_id, prompt_id, provider)` on `arq:pipeline` — fills the Phase 7 stub.
- [x] **Task 3** — `backend/app/llm/google.py` / `groq.py`: finalize execution call sites — `gemma-4-31b-it` (no tools) and `openai/gpt-oss-120b` (`tools=["web_search"]`), capturing `citations` from Groq's web-search loop into `ai_responses.citations`.
- [x] **Task 4** — `backend/app/api/v1/sources.py`: `GET /scans/{id}/sources` — the citation-aggregation SQL from §7.8 (`jsonb_array_elements(citations)` grouped by domain, top 20).
- [x] **Task 5** — Retry/failure policy per §13.1 wired into `execution.py`: 429 → cooldown + next-key (no ARQ retry consumed), 5xx/timeout → exponential backoff via Phase 2's router, pool breaker open → `status='skipped'` (§13.3's execution-side asymmetry — skip, don't defer).
...
- [x] **Task 1** — `backend/app/services/entity_resolution.py`: complete the match-order algorithm from §11 — exact `name_norm` → exact alias → domain-in-text/citations → fuzzy (`rapidfuzz.token_set_ratio ≥ 88`) → unmatched (`entity_id=null`). Word-boundary matching (`\bnotion\b`), short-name (`≤3` chars) exact-only rule, discovered-name dedupe/fuzzy collapse pass over the `entity_id IS NULL` set.
- [x] **Task 2** — `backend/app/services/evaluation.py`: Stage A — string-match `raw_response` against `scan_entities` via `entity_resolution.py` → `target_mentioned` + known-competitor mentions. Stage B — `llama-3.3-70b-versatile` via the `eval_a`/`eval_b` routing rule (`eval_a` if `provider == 'google_ai_studio'` else `eval_b`), structured JSON via Groq JSON mode + Pydantic validation + one repair re-prompt on parse failure, target-not-mentioned short-circuit (shorter prompt/output; call still happens for `mentioned_companies`).
- [x] **Task 3** — `backend/app/llm/prompts/evaluation.jinja`: the evaluator prompt, explicitly scoping `sentiment` to the target company **as discussed in this response**, not overall tone, per §7.9.
- [x] **Task 4** — `backend/app/worker/jobs.py`: `evaluate_response(ctx, response_id)` on `arq:pipeline` — writes `evaluations` (Stage A authoritative for `target_mentioned`/known-competitor mentions, Stage B authoritative for sentiment/rank/recommendation, neither overwrites the other), fans out `mentions`, `DECR pending_eval`, enqueues `aggregate_scan` at 0 with `_job_id=f"agg:{scan_id}"` (stub — Phase 10 fills in the body).
- [x] **Task 5** — Failure policy per §13.3: pool breaker open on an eval call → **defer** (`_defer_by`, exponential up to `EVAL_MAX_DEFER_S=900`), distinct from execution's skip-on-breaker-open.
...
- [x] **Task 1** — `backend/app/services/aggregation.py`: every formula in §12 as pure SQL/SQLAlchemy over `mentions`/`evaluations`/`ai_responses` — AI Visibility, Recommendation Rate (+ when-mentioned variant), Share of Voice (denominator includes discovered companies), Overall Sentiment, Competitor Mention Frequency, Average Rank (+ `n`), Rank Distribution, Prompt Category Performance, Provider Comparison (+ per-provider `success_rate`), Top Sources. Assembles the exact JSON shape from §12 into `scan_metrics.metrics`.
- [x] **Task 2** — `backend/app/worker/jobs.py`: `aggregate_scan(ctx, scan_id)` on `arq:pipeline` — the authoritative-SQL-over-counter guard from §8 (`db.scan_counts` check, re-defer if `evaluated < total` and deadline not passed), then computes + **upserts** `scan_metrics` (idempotent).
...
- [x] **Task 4** — `backend/app/api/v1/scans.py`: `POST /scans/{id}/retry` — re-runs only failed/skipped `ai_responses`/`evaluations` rows, relying on the unique constraints for safety per §13.2.
...
- [x] **Task 1** — `backend/app/api/v1/dashboard.py`: `GET /scans/{id}/dashboard` — one read of `scan_metrics.metrics`, no read-time aggregation.
- [x] **Task 2** — `backend/app/api/v1/prompts.py`: `GET /scans/{id}/prompts` (paginated, filters `category`/`provider`/`sentiment`/`mentioned`), `GET /scans/{id}/prompts/{pid}` (both providers' responses + evaluations + citations joined).
- [x] **Task 3** — `backend/app/main.py`: mount `profiles.py`, `dashboard.py`, `prompts.py`, `sources.py`; confirm every endpoint in §6's table now exists.
- [x] **Task 4** — `backend/tests/fixtures/full_scan.py`: one reusable fixture seeding a complete scan (companies → scan → profile → entities → prompts → responses → evaluations → mentions → scan_metrics). Build once here; reused by this phase's tests and by Phase 21's end-to-end test.
...
- [x] **Task 1** — `backend/app/worker/jobs.py`: `sweep_stalled_scans(ctx)` per §13.4 — queries scans stuck in `executing`/`evaluating`/`aggregating` for >10 min, `reconcile()`s each by recomputing truth from Postgres and re-enqueuing missing jobs via their deterministic `_job_id`s (no-op if already queued).
- [x] **Task 2** — `backend/app/worker/settings.py`: register the sweeper as an ARQ cron on `InteractiveSettings` (every 2 min) per §8.
- [x] **Task 3** — `backend/app/core/locks.py`: `lock:scan:{id}:advance` SETNX lock per §9, used anywhere two paths could double-advance the state machine (sweeper vs. a live job finishing concurrently).
- [x] **Task 4** — Thread `scan:{id}:cancelled` checks (already in `execute_prompt`) through `evaluate_response` and `aggregate_scan` too, so `DELETE /scans/{id}` actually stops in-flight work between steps.
- [x] **Task 5** — `backend/app/core/cost.py`: `cost:daily` Redis counter + `SCAN_COST_CEILING_USD` check invoked from Phase 2's cost-tracking decorator; aborts with `COST_CEILING_EXCEEDED` when either ceiling is hit.
...
- [x] **Task 1** — `backend/app/main.py`: CORS allowlist restricted to the exact Vercel domain (not `*`); confirm `X-API-Key` middleware covers every mounted router except `/health`.
- [x] **Task 2** — `backend/app/main.py`: `GET /health` extended to ping Postgres + Redis per §3 (still no auth required).
- [x] **Task 3** — `backend/app/core/logging.py`: `structlog` setup emitting `scan_id`, `job_name`, `provider`, `attempt` on every line; `scan_id` as the correlation ID from first HTTP request through last evaluation.
- [x] **Task 4** — Sentry SDK wired into `main.py` (API) and `worker/settings.py` (worker).
- [x] **Task 5** — `backend/migrations/versions/e4d48cf2c05b_ops_view.py`: plain Postgres views for §16's ops dashboard (scans by status, p50/p95 duration, per-provider success rate/p95 latency, cost per scan). Breaker trips live only in Redis (`app/core/circuit.py`), not derivable from a Postgres view — excluded, dashboard reads those directly from Redis.
- [x] **Task 6** — Checklist pass: `PRICING_USD_PER_1K` (Phase 2) and every `MODEL_*` env var (§17) confirmed 1:1 — all four models present, unknown-model lookup still raises rather than silently costing $0. Rates remain placeholder `0.000` pending real verification, as the spec itself flags as an open item.
- [x] **Task 7** — `backend/app/config.py`: final pass confirming every var in §17 is present with correct defaults (`ADAPTIVE_RATE_LIMIT=true`, `SCAN_SUCCESS_THRESHOLD=0.70`, `EVAL_MAX_DEFER_S=900`, etc.); added `sentry_dsn` (not enumerated in §17's block, but required by Task 4).
...
- [x] **Task 1** — `frontend/src/lib/api.ts`: thin fetch wrapper injecting `X-API-Key` (`VITE_API_KEY`) and base URL (`VITE_API_URL`) on every call, parsing `{error:{code,message,details}}` into a typed `ApiError`.
- [x] **Task 2** — `frontend/src/lib/types.ts`: hand-written TypeScript types mirroring §4/§12's shapes (`Scan`, `CompanyProfile`, `ScanEntity`, `Prompt`, `DashboardMetrics`, etc.) — no codegen, no OpenAPI spec to generate from.
- [x] **Task 3** — `frontend/src/hooks/api.ts`: one query/mutation hook per endpoint group (`useResolveCompany`, `useCreateScan`, `useScan`, `useProfile`, `usePatchProfile`, `useConfirmProfile`, `useScope`, `useLaunch`, `useDashboard`, `usePrompts`), all keyed on `scan_id` per §6.1. No global store — TanStack Query's cache is the store.
- [x] **Task 4** — `frontend/src/routes/scans.$id.tsx`: scaffold the `/scans/:id/...` route tree per §6.1 (individual route files added per-page in Phases 15–20).
...
- [x] **Task 1** — `frontend/src/routes/index.tsx`: name+website form calling `POST /companies/resolve` then `POST /scans`. `COMPANY_MISMATCH` is a **hard** `422` (per the resolved gap) — surfaced as a blocking form error, not a "continue anyway" affordance.
- [x] **Task 2** — same route: if `POST /scans` returns `{reused: true}` or an active/recent scan, redirect straight to that scan's current page based on its `status` via the new `frontend/src/lib/status.ts` (`statusToPath`) — Phase 20 finalizes wiring this same map into every other route's loader.

**Dependency note:** Needs Phase 14's client + Phase 3's backend live.

**Testing checkpoint:** Verified live against the real local backend (Phase 21's real-provider constraint means enrichment itself isn't exercised here — a profile row was seeded directly to reach `awaiting_verification`, mirroring what `enrich_company` writes): submitting a valid company creates a scan and redirects correctly by status; `ApiError` with `code=COMPANY_MISMATCH` renders the blocking inline copy from §6.1. Full redirect-to-Verification path re-verified together with Phase 16 below.
...
- [x] **Task 1** — `frontend/src/routes/scans.$id.verify.tsx`: editable sections for industry/products/competitors/aliases (add/edit/remove); company/website rendered **read-only**. `PATCH` on save, `POST /profile/confirm` on confirm. (`GET /scans/{id}` was extended with `company_name`/`company_domain` — a genuine gap, not scope creep: no existing endpoint exposed the locked company identity this page's spec requires as read-only display.)
- [x] **Task 2** — same route: render verifier `issues` inline with keep/remove per flagged item after the first confirm returns `issues_found` (polls `GET /scans/{id}` while `status=verifying`, refetches the profile once it settles), then a second confirm button to accept as-is.

**Dependency note:** Needs Phase 14 + Phase 5's backend live.

**Testing checkpoint:** Verified live end-to-end against the real backend (Notion fixture, real Postgres): created a scan, seeded a v1 profile + `awaiting_verification` directly (enrichment itself needs real LLM keys, reserved for Phase 21 — this mirrors exactly what `enrich_company` would have written), confirmed `GET /scans/{id}` returns `company_name`/`company_domain`, `PATCH /profile` persists edits as v2 `user_edited`, seeded a verifier issue (`{field,value,reason}`, matching the rendered shape) then called `POST /profile/confirm` — took the synchronous "accept as-is" path exactly as the second-confirm button would trigger, wrote v3 `ai_verified`, and the scan status flipped to `scope_pending`. Reload-mid-flow is covered by the redirect-guard `useEffect` reading `GET /scans/{id}` on mount (same mechanism proven above).
...
- [x] **Task 1** — `frontend/src/routes/scans.$id.scope.tsx`: checkbox list of the 9 PRD monitoring categories, all checked by default, `PUT /scans/{id}/scope` on submit, navigate to Progress.

**Dependency note:** Needs Phase 6's backend endpoint.

**Testing checkpoint:** Verified live against the real backend: seeded a scan directly at `scope_pending` (no LLM needed for this endpoint), unchecked a category via `PUT /scope` — `monitoring_categories` matched the selection exactly; re-`PUT` after `POST /launch` advanced the status returned `409 INVALID_STATE_TRANSITION` with the expected error envelope.
...
- [x] **Task 1** — `frontend/src/routes/scans.$id.progress.tsx`: `POST /scans/{id}/launch` button, then a 2s `GET /scans/{id}` poll (TanStack Query `refetchInterval`) driving a progress bar off `progress.{stage, done, total}`; cancel button wired to `DELETE /scans/{id}`; auto-navigate to Dashboard on terminal status. Note: `scope_pending` is handled by this page (renders the Launch button) rather than redirecting to Scope, since `PUT /scope` doesn't itself advance `status` — only `POST /launch` does; the Scope page is only the cold-entry target for that status.
...
**Testing checkpoint:** Contract-level verification against the real backend (no worker/real LLM keys in this dev environment — full pipeline run deferred to Phase 21's real-provider pass, consistent with every earlier phase): seeded a scan at `scope_pending`, confirmed the page's status set renders the Launch button; `POST /launch` returned `202` with `status=queued`, matching the polling condition; `DELETE /scans/{id}` returned `204` and the cooperative-cancellation contract (status doesn't flip synchronously for an in-flight job, per §13) matched what the cancel button expects. Full bar-movement + auto-navigate-to-Dashboard behavior will get real exercise once Phase 21 runs an actual pipeline.
...
- [x] **Task 1** — `frontend/src/routes/scans.$id.dashboard.tsx`: executive summary tiles, leaderboard, competitor comparison (or "Discovered competitors" when `brand_only`), sentiment breakdown, prompt category performance, provider comparison, top sources — all from the single `GET /scans/{id}/dashboard` payload. Use `recharts` (already installed), no new charting dependency.
- [x] **Task 2** — same route: render the `completed_with_gaps` banner and single-vs-two-provider stamp (§13.2) when applicable.

**Dependency note:** Needs Phase 11's `GET /dashboard` and a completed scan to point at.

**Testing checkpoint:** Verified live against the real backend using Phase 11's `seed_full_scan` fixture directly (ran outside pytest against the real dev DB, then cleaned up): a normal `completed` scan and a `brand_only=True, completed_with_gaps` scan both seeded, `GET /dashboard` responses matched `DashboardMetrics` field-for-field, and both scan pages SSR'd (200, no server crash) against the real payload shapes. A real completed pipeline run (not just the fixture) is exercised in Phase 21.
...
- [x] **Task 1** — `frontend/src/routes/scans.$id.prompts.tsx`: paginated list from `GET /scans/{id}/prompts` with `category`/`provider`/`sentiment`/`mentioned` filters; row expansion (in place, not a separate detail route) calling `GET /scans/{id}/prompts/{pid}` for prompt text, both providers' responses, evaluations, and mentioned companies.
- [x] **Task 2** — extracted the redirect-guard duplicated across Verify/Scope/Progress/Dashboard into `frontend/src/hooks/useStatusGuard.ts` (one shared hook wrapping `statusToPath`), then added the same guard to the new Prompts page — this is the "finalized status → page map across all six routes" in practice, since each page's own `useEffect` (not a central `router.tsx` loader) is what actually redirects on reload; `router.tsx` itself has no per-route logic to hold this.

**Dependency note:** Needs Phase 11's paginated endpoints.

**Testing checkpoint:** Verified live against the real backend using the `seed_full_scan` fixture: `GET /prompts` unfiltered returned 4 items, filtered by `category=informational&provider=groq&mentioned=true` narrowed to 1, matching `PromptList`; `GET /prompts/{pid}` returned both providers' `raw_response`/`evaluation`/`citations`, matching `PromptDetail` field-for-field; the page SSR'd without error. The guard mechanism (redirect away from a page that doesn't match the current status) was verified by code path and by the identical pattern already proven live on Verify/Scope/Dashboard in earlier phases; exercising it against a real in-flight `executing` scan in a browser is deferred to Phase 21 (no browser tool available in this environment, and no real pipeline run exists yet outside Phase 21's real-provider pass).
...
- [x] **Task 1** — `render.yaml`: `brandmon-api` (web service, `uvicorn app.main:app --port $PORT`, health check `/health`), `brandmon-worker-pipeline` (background worker, `arq app.worker.settings.PipelineSettings`, **paid instance**), `brandmon-worker-interactive` (background worker, `arq app.worker.settings.InteractiveSettings` + sweeper cron) — per §3. Secrets grouped in an `envVarGroups` block, all marked `sync: false` (filled in via the Render dashboard, never committed).
- [ ] **Task 2** — **Blocked, needs you:** provisioning real keys into Render's dashboard requires a Render account and real Google AI Studio / Groq API keys — I don't have either. Once you have a Render project and the 11 keys, this is just pasting them into the `brandmon-secrets` env var group `render.yaml` already defines, plus the Supabase pooler connection string (port 6543).
- [ ] **Task 3** — **Blocked, needs you:** same reason — Vercel project creation and env var configuration requires your Vercel account.
- [x] **Task 4** (code) / **[ ] Task 4 (run)** — `backend/tests/e2e/test_full_pipeline.py` written: chains `enrich_company → verification.accept → generate_prompts → execute_prompt → evaluate_response → aggregate_scan → finalize_scan` against real providers (each job called directly in sequence, matching `test_jobs.py`'s established ctx-construction pattern, since no real ARQ worker runs during pytest), `PROMPT_COUNT` monkeypatched to 4 for cost. Auto-skips via `pytest.mark.skipif` when no real key-pool env vars are present (confirmed: it collects and skips cleanly in this dev environment, `1 skipped`). **Actually running it costs real tokens and needs the keys from Task 2** — run manually once those exist: `cd backend && pytest tests/e2e/test_full_pipeline.py -v`.
- [x] **Task 5** — `backend/tests/golden/cases.py`: 10 hand-labeled fixtures covering clear positive/negative/neutral, unmentioned target, two ranked-list variants, a discovered-unknown-company mention, mixed-sentiment, competitor-only-mentioned, and a no-competitors-named case. `backend/tests/golden/run_golden.py`: manual runner (not pytest-collected, not a CI gate) that runs each case through the real Stage B evaluator and diffs against expected. Also needs real Groq keys to actually run.
- [ ] **Task 6** — **Blocked, needs you:** a QA walk "against the deployed stack" requires an actual deployment (Tasks 2/3). Every page's live-backend contract was already verified individually in Phases 15–20 (Onboarding/Verification/Scope/Progress/Dashboard/Prompt Explorer all exercised against the real local backend); what's left is confirming the same thing works through the real deployed URLs.
- [ ] **Task 7** — **Blocked, needs you:** wiring alerts into Sentry/Render's alerting requires accounts on both, which I don't have access to.
...
**Status:** Everything buildable without external credentials is done (`render.yaml`, the e2e test, the golden set) and verified not to break anything (248 passed, 1 skipped; lint clean; no migration drift). Tasks 2/3/6/7 and actually *running* Task 4/5 are genuinely blocked on things only you can provide: a funded Google AI Studio + Groq key set, and Render/Vercel account access. Not attempted with fabricated credentials or partial/guessed deployment config, per the "hard-to-reverse, external-system" rule — this is real spend and real infra, not something to guess at.

**Testing checkpoint (once unblocked):** Full pipeline reaches a terminal status against real providers at least once; dashboard renders correctly for the real run; cost fuse and rate limiter behave sanely under real Groq/Google traffic (watch TPM per §15.1); deployed Vercel frontend talks to deployed Render backend end-to-end with no CORS/API-key issues.

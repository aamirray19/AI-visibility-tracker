    ...
    async def complete(
        ...
        *,
        ...
        async def raw_call(key: Key) -> LLMResponse:
            return await self._call(key, prompt, system=system, schema=schema, timeout=timeout)

        return await routed_complete(self.redis, self.pool, raw_call)

    async def _call(
        self, key: Key, prompt: str, *, system: str | None, schema: type[BaseModel] | None, timeout: float
    ) -> LLMResponse:
